...
var TestFire = (function(){
    ...
    function start(){
        ...
        var fs_buoy = common +
            ...
            'vec2 decode(vec2 v){ return v*2.0-1.0; }\n' +
            'vec2 encode(vec2 v){ return clamp(v*0.5+0.5, 0.0, 1.0); }\n' +
            'void main(){ vec2 uv=v_uv; float d=texture2D(uDensity,uv).r; vec2 vel=decode(texture2D(uVelocity,uv).xy); vel.y += d * strength * dt * 0.01; gl_FragColor=vec4(encode(vel),0.0,1.0); }\n';
        ...
        addSlider('buoyancy (rise)', 0.0, 150.0, 1.0, buoyancy, function(v){ buoyancy = v; });
        addSlider('advection speed', 0.01, 0.3, 0.005, rdx, function(v){ rdx = v; });
        ...
        function updateSparks(dtSec){
            for(var i=sparks.length-1;i>=0;i--){
                ...
                addSplat(s.x, s.y, s.vx*0.2, s.vy*0.2, 0.006, 1.5*s.life);
            }
        }
