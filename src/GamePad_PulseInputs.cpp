...
void setupPulseInputs()
{
  ...
  Serial.println();
  Serial_INFO;
  Serial.println("🎚 Pulse Inputs via Hardware PCNT Units: " + String(PulseInputs_Count));

  int count = PulseInputs_Count;
  if (count > 4) count = 4; // ESP32-S3 provides up to 4 PCNT units natively
  ...
  for (int i = 0; i < count; i++)
  {
    ...
    // Configure hardware pulse counter unit
    pcnt_config_t pcnt_config = {
        .pulse_gpio_num = pulseInput->Pin,
        .ctrl_gpio_num = PCNT_PIN_NOT_USED,
        .lctrl_mode = PCNT_MODE_KEEP,
        .hctrl_mode = PCNT_MODE_KEEP,
        .pos_mode = PCNT_COUNT_INC,   // Hardware increments on every rising edge
        .neg_mode = PCNT_COUNT_DIS,   // Ignore falling edge or count both if desired
        .counter_h_lim = 32767,
        .counter_l_lim = -32767,
        .unit = (pcnt_unit_t)i,
    ...
    // Clear and start hardware counting immediately
    pcnt_counter_pause((pcnt_unit_t)i);
    pcnt_counter_clear((pcnt_unit_t)i);
    pcnt_counter_resume((pcnt_unit_t)i);

    Serial.printf("Hardware PCNT unit %d bound to pin %d successfully\n", i, pulseInput->Pin);
  }

  Serial.println("Hardware pulse counters initialized");
}
