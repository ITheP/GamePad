...
void InitExternalLED(ExternalLEDConfig *config, CRGB *leds)
{
  ...
  Serial.printf("1st LED: %d, 1st External LED: %d, External Size: %d",
                config->LEDNumber,
                config->ExternalLED,
                config->ExternalLEDs.size());
...
// Hat effects
  ...
  FastLED.show();
  // Serial.printf("Updating %d LEDs on %d strips - ", FastLED.size(), FastLED.count());
  // // Loop through each strip controller
  // for (int i = 0; i < FastLED.count(); i++) {
  //     int ledsOnThisStrip = FastLED[i].size();
  //     Serial.printf("Strip %d has %d LEDs", i, ledsOnThisStrip);
  // }
  // Serial.println();
